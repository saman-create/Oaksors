...
export function GetStartedPage() {
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    const payload: RetirementIntakeSubmission = { firstName: String(data.get("firstName") ?? "").trim(), lastName: String(data.get("lastName") ?? "").trim(), dob: String(data.get("dob") ?? ""), ssn, phone: String(data.get("phone") ?? "").trim(), email: String(data.get("email") ?? "").trim(), address: String(data.get("address") ?? "").trim(), married: data.get("married") === "married" ? "married" : "not_married", portfolioValue: String(data.get("portfolioValue") ?? ""), accountTypes, ...(notes ? { notes } : {}), privacyConsent: true };
              ...
              {submission.phase === "success" ? (
                ...
                <fieldset disabled={submission.isSubmitting}>
                  ...
                  <div className="mp-form-grid">
                    ...
                    <FormField label="Social Security number" name="ssn" type="text" inputMode="numeric" autoComplete="off" placeholder="123-45-6789" minLength={9} maxLength={11} pattern="[0-9]{3}-?[0-9]{2}-?[0-9]{4}" title="Enter 9 digits, with or without dashes." required error={submission.fieldErrors.ssn} />
